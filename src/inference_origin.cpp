...
{
    return accumulate(v.begin(), v.end(), 1, std::multiplies<T>());     //初值为1，std::multiplies<T>() 表示相乘
}
...
int main(int argc, char* argv[])
{
    ...
    const char* inputName = strdup(session.GetInputNameAllocated(0, allocator).get());  // strdup 在堆区开辟内存，并复制传入的指针指向的内容
